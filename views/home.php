<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
    <link rel="stylesheet" href="https://pro.fontawesome.com/releases/v5.10.0/css/all.css" integrity="sha384-AYmEC3Yw5cVb3ZcuHtOA93w35dYTsvhLPVnYs9eStHfGJvOvKxVfELGroGkvsg+p" crossorigin="anonymous"/>
    <link rel="stylesheet" href="home.css">
    <title>Page d'accueil</title>
</head>
<body>
    <div>
        <?php include('../header_footer/header.php'); ?>
        <!-- <link rel="stylesheet" href="../header_footer/style.css"> -->
    </div>
<section class="container-fluid my-5 presentation">
    <div class="container">
        <div class="row gy-5 mx-5 tab">
            <div class="col-12 col-md-5 offset-md-1">
                <img src="../images/logo.jpeg" alt="Image de l'entreprise" class="img-fluid img-thumbnail">
            </div>
            <div class="col-12 col-md-5">
                <h2 class="fs-3 bg-danger text-white fw-bold text-uppercase">Qui sommes-nous ?</h2>
                <p class="text-wrap para">
                    Loire sanous sécurité est une entreprise privée de prévention, 
                    gardiennage et de sécurité des personnes et des biens. 
                    Elle met à votre disposition son savoir-faire et son expérience 
                    pour couvrir tous besoins en matières de sécurité <a href="about.php">lire la suite ...</a>
                </p>
            </div>
        </div>
    </div>
</section>
<section class="container-fluid my-3 team">
    <div class="container">
        <div class="row gy-3 mx-5 tab">
            <div class="col-12 col-md-5 offset-md-1">
                <h2 class="fs-3 bg-danger text-white fw-bold text-uppercase">Nos équipes</h2>
                <p class="text-wrap para">
                    Loire Sanous Sécurité met à vos services une équipe de professionnels 
                    qualifiés en fonction de votre demande. Tous nos agents ont la carte 
                    professionelle et se conforment au respect des dispositions 
                    législatives et réglementaires, notamment les activités privées de sécurité.
                </p>
            </div>
            <div class="col-12 col-md-5">
                <img src="../images/team.jpg" alt="" class="img-fluid shadow">
            </div>
        </div>
    </div>
</section>
<section class="container-fluid security my-2">
    <div class="container mission shadow">
        <div class="container vision">
            <h2 class="fs-4 text-center">Votre sécurité est notre priorité</h2>
            <p class="text-wrap text-center">Nous considérons la sécurité comme une préoccupation principale et mettons
                les meilleures façons de gérer le feu avec beaucoup de technologies
            </p>
        </div>
        <div class="row container g-3">
            <div class="col-12 col-md-3 mx-1 content border rounded-3">
                <p class="icon text-center"><i class="fas fa-rocket fa-2x"></i></p>
                <p class="fs-5">Notre mission</p>
                <p class="text-wrap paragraph">
                    Notre mission est d’assister
                    rapidement aux besoins de 
                    protection de feu de nos 
                    clients basé sur la nature de 
                    leurs business
                </p>
                <button class="btn-lien" data-link="0">
                    <a href="#" class="text-decoration-none">Voir plus</a>
                </button>
            </div>
            <div class="col-12 col-md-4 mx-1 content border rounded-3">
                <p class="icon text-center"><i class="far fa-eye fa-2x"></i></p>
                <p class="fs-5">Notre vision</p>
                <p class="text-wrap paragraph">
                    Notre vision est de devenir
                    l’un des leaders dans le 
                    système de protecttion 
                    contre le feu en France et 
                    faire en sorte que les gens
                    s’intéressent aux services 
                    que nous offrons
                </p>
                <button class="btn-lien" data-link="0">
                    <a href="#" class="text-decoration-none">Voir plus</a>
                </button>
            </div>
            <div class="col-12 col-md-3 mx-1 content border rounded-3">
                <p class="icon text-center"><i class="far fa-star fa-2x"></i></p>
                <p class="fs-5">Nos valeurs</p>
                <p class="text-wrap paragraph">
                    Notre succès repose sur un
                    ensemble distinct de 
                    valeurs qui traverse toutes 
                    les diverses activités 
                    commerciales
                </p>
                <button class="btn-lien" data-link="0">
                    <a href="#" class="text-decoration-none">Voir plus</a>
                </button>
            </div>
        </div>
    </div>
</section>
<section class="container-fluid secteur my-2">
    <div class="container secteur" id="secteur">
        <h2 class="fs-4 text-center">Nos secteurs d’activité & clients</h2>
        <p class="text-wrap text">
            Usines, entrepôts, parcs, zones industrielles, chantiers, immeubles de bureaux, sites stratégiques, sièges sociaux, parcs d’affaires, parking.
            Magasins, centres commerciaux, entrepôts, centres logistiques
            Administrations, services publics, hôpitaux, banques, stades, salons, soirées, vernissages, réceptions, foires & expositions, manifestations
            sportives, commerciales ou culturelles, discothèques, restaurant, mariage, campings
        </p>
    </div>
    <div class="container image">
        <img src="../images/petrol.jpg" alt="" class="img-fluid">
    </div>
</section>
<section class="container-fluid activity my-5">
    <div class="container">
        <h2 class="fs-3 text-center">Nos activités & prestations</h2>
        <div class="row g-3">
            <div class="col-12 col-md-4">
                <div class="card">
                    <img src="../images/filtre.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <div class="card-text">
                            FILTRAGE
                        </div>
                        <button class="btn-primary button" data-tabtitle="0">
                            <a href="#">Voir plus</a>
                        </button>
                        <div id="tab0" class="onglet active">
                            <p class="fw-light text-wrap fs-3">
                                <h4 class="fw-bold text-danger text-start">FILTRE</h4>
                                - Contrôle d'accès et de sorties <br>
                                - Agent de sécurité et la gestion des visiteurs du site : 
                                  identité, badges, clés, ouvertures de barrières / porte,
                                  orientation des visiteurs
                            </p>
                        </div>
                    </div>
                </div>
                </div>
            <div class="col-12 col-md-4 col-sm-12">
                <div class="card">
                    <img src="../images/alar.jpg" class="card-img-top" alt="...">
                    <div class="card-body">
                        <div class="card-text">
                            GESTION DU TRAFIC ET ALARME
                        </div>
                        <button class="btn-primary button" data-tabtitle="1">
                            <a href="#">Voir plus</a>
                        </button>
                        <div id="tab1" class="onglet">
                            <p class="fw-light text-wrap fs-3">
                                <h4 class="fw-bold text-danger text-start">GESTION DU TRAFIC & ALARME</h4>
                                - Maintien de la fluidité et de sécurité du trafic interne du site <br>
                                - Faire respecter les normes ou consignes de circulation et de stationnement <br>
                                - Orientation des visiteurs <br>
                                - Installation des alarmes
                            </p>
                        </div>
                    </div>
                </div>
                </div>
            <div class="col-12 col-md-4 col-sm-12">
                <div class="card">
                    <img src="../images/sur.jpg" class="card-img-top img" alt="...">
                    <div class="card-body">
                        <div class="card-text text-start text-wrap-none" >
                            GESTION ET GARDIENNAGE
                        </div>
                        <button class="btn-primary button" data-tabtitle="2">
                            <a href="#">Voir plus</a>
                        </button>
                        <div id="tab2" class="onglet">
                            <p class="fw-light text-wrap fs-3">
                                <h4 class="fw-bold text-danger text-start">GARIDENNAGE &  SURVEILLANCE</h4>
                                - Surveillance de magasin : Arrière caisse <br>
                                - Surveillance de sites industriels, chantiers, entreprises, banques, ERP, IGH,
                                sociétés de service, télésurveillance ... <br>
                                - Traitement des anomalies et rapports ... 
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="row g-3 my-2">
            <div class="col-12 col-md-6 col-sm-12">
                <div class="card">
                        <img src="../images/intervention.jpg" class="card-img-top img" id="img" alt="...">
                        <div class="card-body">
                            <div class="card-text">
                                INTERVENTION ET RONDES
                            </div>
                            <button class="btn-primary button" data-tabtitle="3">
                                <a href="#">Voir plus</a>
                            </button>
                            <div id="tab3" class="onglet">
                                <p class="fw-light text-wrap fs-3">
                                    <h4 class="fw-bold text-danger text-start">INTERVENTION & RONDES</h4>
                                    - Protection de sites <br>
                                    - Intervention SSI <br>
                                    - Sécurité incendie <br>
                                    - Mission d'urgence et évacuation <br>
                                    - Postes alarmes, les ERP et les IGH <br>
                                    - Rondes de surveillance <br>
                                    - Prévention contre le vol <br>
                                    - Contrôle des fermetures des issues
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-12 col-md-6">
                    <div class="card">
                        <img src="../images/sec.jpg" class="card-img-top img" alt="...">
                        <div class="card-body">
                            <div class="card-text">
                                SECURITE INCENDIE (SSIAP)
                            </div>
                            <button class="btn-primary button" data-tabtitle="4">
                                <a href="#">Voir plus</a>
                            </button>
                            <div id="tab4" class="onglet">
                                <p class="fw-light text-wrap fs-3">
                                    <h4 class="fw-bold text-danger text-start">SECURITE INCENDIE</h4>
                                    - La prévention des incendies <br>
                                    - L'alerte et l'accueil des secours <br>
                                    - L'assistance aux personnes <br>
                                    - Le management de l'équipe de sécurité <br>
                                    - La prévision technique encadrée par les règlements de sécurité <br>
                                    - Le management du service de sécurité incendie <br>
                                    - Le conseil du chef d'établissement en matière de sécurité incendie <br>
                                    - L'organisation des secours internes ... 
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<div class="mb-0">
    <?php include('footer.php'); ?>
</div>
<script src="../header_footer/header.js"></script>
<!-- <script src="menu.js"></script> -->
</body>
</html>